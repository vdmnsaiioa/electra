...
def equivariance_check(model, train_loader):
    ...
    translation = torch.randn(3, device=model.device)*25
    ...
    if checks[1]:
        with torch.no_grad():
            ...
            trans_diff = torch.mean(torch.abs(fgx_trans_z - pred_base) / torch.min(torch.abs(fgx_trans_z) + 1e-6, torch.abs(pred_base) + 1e-6))
