...
class CustomVaspChargeDensity:
    ...
    def read(self, filename):
        ...
        while True:
            ...
            chg = np.empty(ng)
            charge_dens = self.READ_CHG_CUSTOM(fd, chg, atoms)
            self.chg.append(charge_dens)
 ...
 %#11.5G %#11.5G %#11.5G %#11.5G %#11.5G\n' % chgtmp[ii * 10:(ii + 1) * 10])
    ...
    def READ_CHG_CUSTOM(self, filename, chg, atoms):
        """Read charge from file object
        ...
        with open(filename.name, 'r') as file:
            ...
            # Skip the first 1+n_atoms lines
            for _ in range(10 + len(atoms)):
                next(file)

            # Read the remaining lines into a single string
            data_str = file.read()
        end_pos = data_str.find("augmentation")
        numeric_data_str = data_str[0:end_pos]
        numeric_data = np.fromstring(numeric_data_str, sep=' ', dtype=np.float64)
        chg = numeric_data.reshape((chg.shape[2], chg.shape[1], chg.shape[0]))
        chg = np.transpose(chg, (2, 1, 0))
        # Normalize by volume
        #chg = np.transpose(chg, (2, 1, 0))
        chg /= volume
        return chg
