...
def load_electra_mm_class():
    ...
    loader = SourceFileLoader("electra_mm_module_simple", str(module_path))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compute per-molecule validation energy errors and aggregate MAE/RMSE for ELECTRA_mm."
    )
    parser.add_argument(
        "--config",
...
def emit_metrics(metrics: dict[str, float], metrics_file: Path | None) -> None:
...
def load_validation_config(config_path: str | os.PathLike[str]) -> dict:
...
def evaluate_loaded_model_on_validation(
    ...
    *,
    ...
    if not config.get("load_model", False):
        raise ValueError("Config must have load_model: True for checkpoint validation.")
    if not validation_files:
        raise ValueError("No validation molecules were resolved from the datasplit JSON in config.")
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(model.val_dataloader()):
            _, qm9_mol, qm9_n_elec, _, mol_cd_file, _, *rest = batch[0]
            ...
            print(f"{sample_name}: |target energy - estimated energy| = {abs_error}", flush=True)
            print(f"{sample_name}: (target energy - estimated energy)^2 = {squared_error}", flush=True)
