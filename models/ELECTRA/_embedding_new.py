...
from modules.aufbau import AufEmbedding, embed_atoms
import einops
...
class TensorEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.distance_proj = lambda w, x: torch.matmul(einops.rearrange(w, "a (b c) -> a b c", b=x.shape[-1], c=x.shape[-1]), x[..., None])[..., 0]


        self.dp1_weights = nn.Sequential(
            nn.Linear(2*units, 2*units),
            nn.SELU(),
            nn.Linear(2*units, int(degree_rbf**2)),
        )
        self.dp2_weights = nn.Sequential(
            nn.Linear(2 * units, 2 * units),
            nn.SELU(),
            nn.Linear(2 * units, int(degree_rbf**2)),
        )
        self.dp3_weights = nn.Sequential(
            nn.Linear(2 * units, 2 * units),
            nn.SELU(),
            nn.Linear(2 * units, int(degree_rbf**2)),
        ...
        self.emb = AufEmbedding(units)
        self.neighbor_emb = NeighborEmbedding(ntypes_node, units, degree_rbf, cutoff, dtype)
        #        self.act = activation()
        self.attention = MultiheadAttention(embed_dim=self.units,
                                            num_heads=self.units//8)
        self.act = activation
    ...
    def reset_parameters(self):
        ...
        for layer in self.dp1_weights:
            if hasattr(layer, "reset_parameters"):
                layer.reset_parameters()
        for layer in self.dp2_weights:
            if hasattr(layer, "reset_parameters"):
                layer.reset_parameters()
        for layer in self.dp3_weights:
            if hasattr(layer, "reset_parameters"):
                layer.reset_parameters()
    ...
    def forward(self, g: dgl.DGLGraph, state_attr: torch.Tensor | None = None):
        ...
        Z = embed_atoms(atom_numbers=g.ndata["atom_numbers"],
                        embedding_dim=self.units,
                        auf_embedding=self.emb,
                        device=g.device)
        Z = self.neighbor_emb(g.ndata["atom_numbers"], Z, g.edata["edge_index"].transpose(0, 1), edge_weight, edge_attr)
        Z, _ = self.attention(Z, Z, Z)
        C = cosine_cutoff(edge_weight, self.cutoff)
        edge_vec = g.edata["bond_vec"]
        bond_invs = Z[g.edata['edge_index']]
        bond_invs = einops.rearrange(bond_invs, "a b c->a (b c)")
        dp1_weights = self.dp1_weights(bond_invs)
        dp2_weights = self.dp2_weights(bond_invs)
        dp3_weights = self.dp3_weights(bond_invs)
        W1 = self.distance_proj(dp1_weights, edge_attr) * C.view(-1, 1)
        W2 = self.distance_proj(dp2_weights, edge_attr) * C.view(-1, 1)
        W3 = self.distance_proj(dp3_weights, edge_attr) * C.view(-1, 1)
        ...
        return X, edge_feat, state_feat, Z
...
class NeighborEmbedding(nn.Module):
    def __init__(
        ...
        """
        super().__init__()
        self.embedding = nn.Embedding(ntypes_node, hidden_channels, dtype=dtype)
