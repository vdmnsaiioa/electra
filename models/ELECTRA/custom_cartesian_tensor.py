...
class CartesianTensor(o3.Irreps):
    ...
    def __new__(
        ...
        ret.formula = formula
        ret.indices = indices
        ret._cached_rtp = rtp  # Cache the rtp instance
        return ret
    ...
    def get_cached_rtp(self, data: Optional[torch.Tensor] = None) -> o3.ReducedTensorProducts:
        if data is not None:
            self._cached_rtp = self._cached_rtp.to(device=data.device, dtype=data.dtype)
        return self._cached_rtp
