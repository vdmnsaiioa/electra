...
class TensorNetInteraction(nn.Module):
    ...
    def __init__(
        ...
        self.cutoff = cutoff
        self.si_params = nn.Parameter(torch.ones(10), requires_grad=True)
    ...
    def forward(self, g: dgl.DGLGraph, X: Tensor):
        ...
        with g.local_scope():
            ...
            # normp1 = (tensor_norm(scalars + skew_metrices + traceless_tensors))[..., None, None]
            ...
            si_param_ctr = 0
            for i in range(4):
                dX = self.si_params[si_param_ctr]*torch.matmul(dX, dX) + self.si_params[si_param_ctr+1]*dX
                si_param_ctr += 2
            X = X + dX
            # X = dX + torch.matmul(dX, dX)
        return X
