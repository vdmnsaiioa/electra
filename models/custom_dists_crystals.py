...
from torch.func import vmap  # PyTorch vmap (experimental in some versions)
...
class MixtureSameFamily(Distribution):
    ...
    def log_prob(self, x, links):
        ...
        if self.scal_mults is not None:
            ...
            # Precompute common Gaussian parameters:
            ...
            gamma = A.inverse().reshape(-1, 9) / 2  # shape: (M, 9)
            mu = self.component_distribution.loc.to(torch.float32)  # shape: (M, 3)
            M = mu.shape[0]
            N = x.shape[0]

            # Precompute the weight tensor (shape: (M,))
            weight_tensor = (self.scal_mults * torch.exp(log_mix_prob) *
                 torch.exp(-half_log_det) * (2 * torch.pi) ** (-3 / 2))

            if method == 'all_images':
                # --- 27-Image Convention ---
                ...
                wt = weight_tensor.view(M, 1)  # shape: (M, 1)
                contrib_per_shift = K_ij @ wt  # shape: (27, N, 1)
                weighted = contrib_per_shift.sum(dim=0).squeeze(-1)  # shape: (N,)

            elif method == 'mic':
                # ------------------ Precompute Effective Gamma ------------------
                # We want to compute, for each Gaussian j:
                #   gamma_tilde_j = flatten(cell^T @ gamma_j_matrix @ cell)
                # where gamma_j_matrix is gamma[j].view(3,3).
                # This is done in PyTorch for all Gaussians.
                # Ensure cell is float32.
                cell = self.cell.to(torch.float32)
                # Compute cell^T once.
                cell_T = cell.t()  # (3,3)
                # Preallocate gamma_tilde of shape (M,9)
                # Reshape gamma from (M,9) to (M,3,3)
                gamma_mat = gamma.view(M, 3, 3)

                # Compute gamma_tilde for each Gaussian:
                # gamma_tilde[j] = cell.T @ gamma_mat[j] @ cell
                gamma_tilde = torch.matmul(torch.matmul(cell.t().unsqueeze(0), gamma_mat),
                                          cell.unsqueeze(0))
                # gamma_tilde now has shape (1, M, 3, 3); squeeze the first dimension:
                gamma_tilde = gamma_tilde.squeeze(0)  # shape: (M, 3, 3)

                # Finally, flatten gamma_tilde to (M,9)
                gamma_tilde = gamma_tilde.reshape(M, 9)
                # Alternatively, if M is large you can vectorize this.

                # ------------------ Convert to Fractional Coordinates ------------------
                # Compute the inverse cell.
                inv_cell = torch.linalg.inv(cell)  # (3,3)
                # Convert x and mu to fractional coordinates.
                x_frac = x @ inv_cell.t()      # (N,3)
                mu_frac = mu @ inv_cell.t()    # (M,3)

                # ------------------ LazyTensor Computation ------------------
                # We want to compute the wrapped difference in fractional space.
                # Let:
                #   diff = x_frac - mu_frac  (using LazyTensors to avoid materializing (N,M,3))
                # Then:
                #   wrapped = diff - diff.round()
                # and the quadratic form will be computed as:
                #   quad = WeightedSqNorm( gamma_tilde, wrapped )
                # which gives d^T * (cell^T gamma cell) * d, i.e. the same as (d @ cell)^T gamma (d @ cell),
                # where d = wrapped difference.
                #
                # Set up LazyTensors:
                x_i = LazyTensor(x_frac.view(N, 1, 3))   # shape: (N,1,3)
                mu_j = LazyTensor(mu_frac.view(1, M, 3))   # shape: (1,M,3)
                diff = x_i - mu_j                        # shape: (N,M,3)
                wrapped = diff - diff.round()            # shape: (N,M,3)

                # Now, gamma_tilde is precomputed in PyTorch (shape (M,9)). We convert it to a LazyTensor:
                gamma_j = LazyTensor(gamma_tilde.view(1, M, 9))  # shape: (1, M, 9)

                # Compute the weighted squared norm:
                # This uses KeOps' built-in method. It expects the "vector" (here, our wrapped difference)
                # to have last dimension = 3 and gamma_j to be of shape (1,M,9).
                sqdist = wrapped.weightedsqnorm(gamma_j)   # shape: (N,M,1)
                K = (-sqdist).exp()                        # shape: (N,M,1)

                # Multiply each kernel value by the corresponding weight (w) and sum over j.
                wt = weight_tensor.view(M, 1)              # shape: (M,1) in PyTorch
                # Using LazyTensor's matrix product operator '@' (which performs a reduction over the j-index).
                contrib = K @ wt    # result is a LazyTensor of shape (N,1)
                weighted = contrib.squeeze(-1)  # final shape: (N,)
            elif method == 'mic_from_all':
                # --- 27-Image Shifts (as in all_images) ---
                lattice_indices = torch.tensor(
                [[i, j, k] for i in [-1, 0, 1]
                           for j in [-1, 0, 1]
                           for k in [-1, 0, 1]],
                ...
                # LazyTensor setup:
                x_i = LazyTensor(x.view(1, N, 1, 3))                   # shape: (1, N, 1, 3)
                mu_j = LazyTensor(mu_shifted.view(27, 1, M, 3))          # shape: (27, 1, M, 3)
                gamma_j = LazyTensor(gamma.view(1, 1, M, gamma.size(1))) # shape: (1, 1, M, 9)

                # Compute weighted squared distances for each shift:
                diff_ij = x_i - mu_j                      # shape: (27, N, M, 3)
                sqdist_ij = diff_ij.weightedsqnorm(gamma_j)  # shape: (27, N, M, 1)

                K_ij = (-sqdist_ij).exp()               # shape: (N, M, 1)

                # Multiply each contribution by the corresponding weight and sum over Gaussians:
                wt = weight_tensor.view(M, 1)             # shape: (M, 1)
                contrib = K_ij @ wt                     # (27, N, 1) via reduction over the M-index
                contrib_all = contrib.squeeze(-1)   # shape: (27, N)
                best_val, best_idx = contrib_all.abs().max(dim=0)
                weighted = contrib_all[ best_idx, torch.arange(contrib_all.shape[1]) ]
        else:
            component_log_prob = self.component_distribution.log_prob(x, links, self.cell)
