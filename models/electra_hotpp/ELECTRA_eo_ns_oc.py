...
class ELECTRA(L.LightningModule, IOMixIn):
    ...
    def __init__(
        ...
        # Once we drop the valence-based slicing we operate directly on the HOTPP
        # scalar channels. Treat those as the working "unit" count so all
        # downstream networks stay dimensionally consistent.
        self.units = config['hotpp_outdim']
        ...
        self.wsm_network = nn.Sequential(
            nn.Linear(3 * self.units, self.units * 3),
            nn.Mish(),
        ...
        self.phi_network = nn.Sequential(
            nn.Linear(self.units, self.units//4), # with only l0 divide by 4
            nn.Tanh(), # changed from minsh
            nn.Linear(self.units //4, self.units//8),
        )
        self.rho_network = nn.Sequential(
            nn.Linear(self.units//8, self.units//8),
            nn.Tanh(), # changed from minsh
            nn.Linear(self.units//8, 1),
        )
        self.energy_layer_norm = nn.LayerNorm(7 * self.units)
    ...
    def init_hotpp_model(self) -> None:
        """Initialise the HOTPP backbone responsible for feature generation."""
        elements = ['H', 'O', 'C', 'N', 'F', 'Cl', 'Br', 'I']
        element_numbers = [atomic_numbers[element] for element in elements]

        embed_type = self.config.get('embed_type', 'atomic')
        if embed_type == 'aufbau':
            emb_layer = AufbauEmbedding(
                element_numbers,
                self.config['master_units'],
                device=self.device,
            )
        elif embed_type == 'atomic':
            emb_layer = AtomicEmbedding(
                element_numbers,
                self.config['master_units'],
            )
        else:
        ...
        cut_fn = PolynomialCutoff(
            cutoff=self.config['hotpp_cutoff'],
            p=self.config['poly_order'],
        )
    ...
    @staticmethod
    def _serialize_energy(value: torch.Tensor | float | int | None):
        if value is None:
            return None
    ...
    def _write_energy_nan_history(self, stage: str) -> None:
        if not self.energy_loss_history:
            return
        log_dir = os.path.dirname(self.energy_nan_log_path)
        if log_dir:
            os.makedirs(log_dir, exist_ok=True)
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())
        with open(self.energy_nan_log_path, "a", encoding="utf-8") as fh:
            fh.write(
                f"Energy loss NaN detected during {stage} at step {int(self.global_step)} on {timestamp}.\n"
            )
            for entry in self.energy_loss_history:
    ...
    def _track_energy_information(
        self,
        *,
        energy_loss: torch.Tensor | None,
        stage: str,
        identifier: str | None,
        batch_idx: int,
        qm9_mol: Atoms | None,
        predicted_energy: torch.Tensor | None,
        target_energy: torch.Tensor | float | None,
    ) -> None:
        ...
        is_nan = torch.isnan(energy_loss_detached).any().item()
        if not is_nan:
            return
    ...
    def forward(self,
        ...
        n_atoms = len(at_numbers)
        n_multiples = torch.full((n_atoms,), self.units, dtype=torch.long, device=self.device)

        # With slicing removed we pass the raw per-atom features through the downstream networks.
        ...
        wsm_ = torch.cat((X_scal, X_vec_scal, X_tens_scal), dim=-1).view(n_atoms, -1)
        scalar_features = X_scal.view(n_atoms, -1)
        vec_features = X_vec.mean(dim=-1)
        ...
        phi_out = self.phi_network(input_l0)
    ...
    def _maybe_log_hotpp_scalars(self, scalar_features: torch.Tensor) -> None:
        ...
        csv_path = output_dir / f"hotpp_scalars_step_{int(step)}.csv"
    ...
    def val_dataloader(self):
        """Validation dataloader mirroring the energy-only dataset pipeline."""
        if self.config['MD_Data']:
            dataset = SmallDensityDataset(
                self.config["MD_ROOT"],
                self.config["MD_MOL"],
    ...
    def test_dataloader(self):
        """Test dataloader for energy predictions only."""
        if self.config['MD_Data']:
            dataset = SmallDensityDataset(
                self.config["MD_ROOT"],
                self.config["MD_MOL"],
    ...
    def training_step(self,
        ...
        if energy is None:
            raise ValueError("Training batch is missing target energy.")
        ...
        base_mse = self.energy_loss_fn(predicted_energy, target_energy)
        loss = (
            base_mse if self.energy_loss_coef is None
            else self.energy_loss_coef * base_mse
        )
        ...
        if atom_count_t is not None:
            nrmse = rmse / atom_count_t
            nrmse_mev = nrmse * self.energy_conversion_factor.to(nrmse.device)
            nl1 = torch.abs(predicted_energy - target_energy) / atom_count_t
            self.log(
                "Train Energy NRMSE",
                nrmse.detach(),
                on_step=True,
    ...
    def validation_step(self,
        ...
        if energy is None:
            raise ValueError("Validation batch is missing target energy.")
    ...
    def test_step(self,
        ...
        if energy is None:
            raise ValueError("Test batch is missing target energy.")
