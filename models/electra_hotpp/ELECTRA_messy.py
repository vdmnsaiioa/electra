...
class ELECTRA(L.LightningModule, IOMixIn):
    ...
    def forward(self,
        ...
        if wrap:
            ...
            # Get the cell as a tensor.
            #pos_disp_final = minimum_image(pos_disp_final, cell_tensor)
    ...
    def validation_step(self,
        ...
        self.log("Validation Density Err %",
                 ...
                 batch_size=1)
        if self.config['hpc']:
            wandb.log({"Validation Density Err %": np.round(100 * density_error.detach().cpu().numpy(), 3)})
        ...
        if self.config['construct_val_cd']:
            if density_error < self.val_error_dict[f"Val_file_{batch_idx}"]:
                ...
                create_chg_delta(
                    ...
                    name_iter_str=f'{mol_cd_file[-17:-11]}_{qm9_mol.get_chemical_formula()}_val_{batch_idx}'
                )
        if batch_idx == 0:
            self.val_iter += 1
        if self.plot_gaus_pos:
            errstr = np.round(100 * density_error.detach().cpu().numpy(), 3)
            ...
            self.plot_gaussian_positions(atom_types=qm9_mol.get_chemical_symbols(),
                                     ...
                                     filename=f'{self.gaus_pos_path_val}/{qm9_mol.get_chemical_formula()}_{errstr}_{file_id_}.html',
    ...
    def test_step(self,
        ...
        if self.config['hpc']:
            wandb.log({"Test Density Err %": np.round(100 * density_error.detach().cpu().numpy(), 3)})
    ...
    def test_dataloader(self):
        if self.config['MD_Data']:
            ...
            return DataLoader(QM9Dataset(self.test_fiels, self.qm9_dens_path, self.config),
                              batch_size=1,
                              shuffle=False,
                              collate_fn=collate_fn,
                              pin_memory=True,
    ...
    def plot_gaussian_positions(self,
        ...
        atom_colors = [atom_color_map.get(atom, 'grey') for atom in atom_types]
        ...
        if simple:
            ...
            gaussian_sizes = np.ones_like(gaussian_sizes)
...
class SmallDensityDataset(Dataset):
    def __init__(self, root, mol_name, split):
        ...
        self.ang_to_bohr = 0.52918
        self.grid_size = 20. * self.ang_to_bohr  # box size in angstrom
    ...
    def _convert_fft(self, fft_coeff):
        ...
        d_shaped = d_flat.view(-1, self.n_grid, self.n_grid, self.n_grid)
        return d_shaped
    ...
    def __getitem__(self, item):
        mol = Atoms(''.join(self.atom_type), positions=self.atom_coords[item]*self.ang_to_bohr)
...
class DifferentiableFloorSTE(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input):
        # In the forward pass, compute the hard floor.
        return torch.floor(input)

    @staticmethod
    def backward(ctx, grad_output):
        # In the backward pass, use an identity gradient (i.e. pretend that the operation was the identity).
        # This is a simple straight-through estimator.
        return grad_output

def differentiable_floor(input):
    return DifferentiableFloorSTE.apply(input)
