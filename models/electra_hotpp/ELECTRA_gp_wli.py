...
class ELECTRA(L.LightningModule, IOMixIn):
    ...
    def __init__(
        ...
        self.phi_network = nn.Sequential(
            nn.Linear(5 * self.units, self.units * 3),
            nn.Mish(),
            nn.Linear(self.units * 3, self.units),
        )
    ...
    def forward(self,
        ...
        print('uaaai',pos_factors,'esti un cacatios')
        ...
        self.pos_disp_text = pos_disp_features
        energy_input = torch.cat((wsm_, pos_disp_features), dim=-1)

        phi_out = self.phi_network(energy_input)
    ...
    def training_step(self,
        ...
        if self.global_step == 0 and not hasattr(self, "_first_batch_written"):
            feature_dict = {
                "pos_features": self.pos_disp_text.detach().cpu().tolist(),
            }
            with open("/home/energy/s234633/ELECTRA/first_batch_features.txt", "w") as f:
        ...
        if self.config['crystal']:
            ...
            if self.config['hpc']:
                if is_ortho:
                    wandb.log({"Orthogonal error %": np_err})
                else:
                    wandb.log({"Non-orthogonal error %": np_err})
            else:
                if is_ortho:
