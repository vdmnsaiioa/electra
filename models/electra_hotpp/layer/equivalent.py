    ...
    def forward(self,
        ...
        for way, linear in enumerate(self.linear_list):
            if way in input_tensors:
                # (ELECTRA): THIS WEIGHTS HAS BEEN ADDED TO MAKE A NONLINEAR VERSION:
...
class TensorProductLayer(nn.Module):
    ...
    def forward(self,
        ...
        for x_way, y_way, z_way in self.combinations:
            ...
            output_tensor = output_tensor * coef
...
class MultiBodyLayer(nn.Module):
    ...
    def forward(self,
        ...
        scalars = input_tensors[0]
        weights_list = [mlp(scalars) for mlp in self.weight_mlp_list]
        ...
        for way, linear in enumerate(self.linear_list):
            ...
            output_tensors[way] = torch.transpose(torch.transpose(linear(tensor), 1, -1) * weights_list[way], 1, -1)
